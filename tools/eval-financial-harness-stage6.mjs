...
function readCases() {
  ...
  return limitArg ? cases.slice(0, Math.max(1, Number(limitArg.split("=", 2)[1]) || cases.length)) : cases;
}
...
async function evalOne(item) {
  ...
  return {
    ...
    planProfiles: plan.map((stage) => stage?.profile).filter(Boolean).join(" > "),
