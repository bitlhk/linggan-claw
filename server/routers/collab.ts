...
export const collabRouter = router({
    ...
    getSettings: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (claw.permissionProfile === "starter") throw new TRPCError({ code: "FORBIDDEN", message: "当前子虾未开通协作权限，请联系管理员处理" });
    ...
    sendRequest: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (requester.permissionProfile === "starter") throw new TRPCError({ code: "FORBIDDEN", message: "当前子虾未开通协作权限，请联系管理员处理" });
